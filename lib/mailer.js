...
// Brevo 通道下发件人必须是 Brevo 后台验证过的 sender。
// 默认发件人改成 Brevo 已验证的 Gmail 账号 hkinsight.hr@gmail.com（Brevo id=1, active=true）。
// 原因：HR_Support@insightelectionhk.com 域名未配 SPF，Brevo 拒绝验证（spfError=true），
//       验证邮件根本发不到企微邮箱。改用 Gmail 后 Brevo 直接放行。
// 如需用回 hr_support（需先在域名 DNS 加 Brevo SPF），设 Render 环境变量 SMTP_FROM_ADDR 覆盖。
const SMTP_FROM_ADDR = process.env.SMTP_FROM_ADDR || (
  BREVO_API_KEY ? 'hkinsight.hr@gmail.com'
    : (SMTP_HOST.includes('resend') ? 'onboarding@resend.dev' : 'hkinsight.hr@gmail.com')
);
// 候选人点"回复"时投到的地址（用人资对外邮箱，方便候选人回复咨询）
